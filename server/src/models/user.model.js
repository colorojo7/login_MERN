...
userSchema.statics.register = async function (email, password, name, DOB) {
  ...
  //* VALIDATIONS *//
      ...
      const passwordStrenght = validator.isStrongPassword(password, strenghtValidatorOptions);
      if (passwordStrenght < minStrenght) {
        return ({error:`Password not strong enough. Strenght ${passwordStrenght}/${minStrenght}. Try lowercase, uppercase, number and simbols`});
      }
...
userSchema.statics.login = async function (email, password){
  ...
  //* VALIDATIONS *//
      ...
      if (!user) {
        throw Error("The e-mail is not in our database");
      }
